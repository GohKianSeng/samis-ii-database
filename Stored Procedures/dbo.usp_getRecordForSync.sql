...
DECLARE @XML XML = (SELECT
	CONVERT(XML, (SELECT CourseID, CONVERT(VARCHAR(22), RegistrationDate,103) AS RegistrationDate, 'Unspecified' AS EnteredBy, B.NRIC AS OriginalNRIC, B.NRIC AS NRIC, B.Salutation, EnglishName, Gender, CONVERT(VARCHAR(10), DOB, 103) AS DOB,
		   ...
		   AddressUnit, Contact, Email, Education, Occupation, Church, ChurchOthers
	...
	CONVERT(XML, (SELECT 'Unspecified' AS EnteredBy, A.NRIC, Salutation, EnglishName, ChineseName, Gender, CONVERT(VARCHAR(10), DOB, 103) AS DOB,
		   ...
		   Sponsor1, Sponsor2, Sponsor2Contact, BaptismByOthers, BaptismChurchOthers, ConfirmByOthers, ConfirmChurchOthers, PreviousChurchOthers
	FROM dbo.tb_members_temp AS A
	INNER JOIN dbo.tb_membersOtherInfo_temp AS B ON A.NRIC = B.NRIC WHERE MarkSync = 1
